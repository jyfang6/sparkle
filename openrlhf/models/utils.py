...
def compute_reward(
    ...
    kl_coef: Union[float, torch.Tensor], 
    ...
    if isinstance(kl_coef, float) and kl_coef <= 0.0:
        kl_coef = 0.0
    ...
    if action_mask is not None:
        ...
        # 下面的是为了记录(我自己加上去的)
        # sequence_kl = (kl * action_mask).sum(dim=1)
        # beta_kl = kl_coef * sequence_kl
        # main_reward = r
        # print("# >>>>> main reward: ", main_reward.float().mean().item(), " sequence kl: ", sequence_kl.mean().item(), " beta kl: ", beta_kl.mean().item(), "\n")
...
def compute_reward_use_recall(
    ...
    if reward_clip_range:
        sequence_reward = sequence_reward.clamp(min=reward_clip_range[0], max=reward_clip_range[1])
        step_reward = step_reward.clamp(min=reward_clip_range[0], max=reward_clip_range[1])

    reward = torch.zeros_like(kl)
    reward += step_reward * action_mask
    eos_indices = action_mask.size(1) - 1 - action_mask.long().fliplr().argmax(dim=1, keepdim=True)
    reward = reward.scatter_add_(dim=1, index=eos_indices, src=sequence_reward.unsqueeze(1).to(kl.dtype))

    reward = reward - kl_coef * kl 
    return reward 
