...
from openrlhf.models.actor_kg_adaptive_rag import ReasoningModel, KGRetriever, ActorKGAdaptiveRAG
from openrlhf.models.model_kg_adaptive_rag import QARewardModel
from openrlhf.datasets.kg_adaptive_rag_dataset import KGAdaptiveRAGDataset, custom_collate_fn
from openrlhf.trainer.ppo_trainer_kg_adaptive_rag import PPOTrainerKGAdaptiveRAG, PPOTrainerKGAdaptiveRAGTreeRollout 
...
def train(args):
    ...
    # load retriever
    retriever = KGRetriever(corpus_path=args.prompt_data)

    # configure model
    # load huggingface model
    actor = ActorKGAdaptiveRAG(
        args.pretrain, 
    ...
    if args.critic_pretrain:
        critic = get_llm_for_sequence_regression(
            ...
            "critic" if args.tree_rollout else "kg_adaptive_critic",
    ...
    reward_model = QARewardModel(data_dir=args.prompt_data, use_recall=args.rm_use_recall, retriever=retriever)
    ...
    if args.init_kl_coef == 0:
        ...
        initial_model = ActorKGAdaptiveRAG(
            args.pretrain, 
            reasoning_model=reasoning_model, 
            retriever=retriever, 
            prompt_max_len=args.prompt_max_len,
        ...
        for p in initial_model.model.parameters():
            p.requires_grad = False 
    
    if args.enable_ema:
        ema_model = ActorKGAdaptiveRAG(
            args.pretrain, 
            reasoning_model=reasoning_model, 
            retriever=retriever, 
            prompt_max_len=args.prompt_max_len,
    ...
    # prepare datasets
    train_data, test_data = blending_datasets(
        args.prompt_data,
    ...
    train_dataset = KGAdaptiveRAGDataset(train_data)
    test_dataset = KGAdaptiveRAGDataset(test_data)

    # prepare dataloader
    train_dataloader = strategy.setup_dataloader(
        train_dataset, args.rollout_batch_size // strategy.world_size, True, True, collate_fn=custom_collate_fn
    )
    test_dataloader = strategy.setup_dataloader(
        test_dataset, args.micro_train_batch_size, True, True, collate_fn=custom_collate_fn
    )
    ...
    # configure Trainer
    trainer_cls = PPOTrainerKGAdaptiveRAGTreeRollout if args.tree_rollout else PPOTrainerKGAdaptiveRAG
    trainer = trainer_cls(
        strategy=strategy,
        actor=actor,
        critic=critic,
    ...
    trainer.fit(args, train_dataloader, test_dataloader, consumed_samples, num_update_steps_per_episodes)
...
if __name__ == "__main__":
    ...
    # Models
    parser.add_argument("--pretrain", type=str, default=None, required=True, help="HF model name or path")
    ...
    # KG Adaptive RAG 
    parser.add_argument("--reasoning_model", type=str, default=None, required=True, help="HF model name or path")
    parser.add_argument("--tree_rollout", action="store_true", default=False) 
    ...
    # whether to use recall in reward model 
    parser.add_argument("--rm_use_recall", action="store_true", default=False)
